...
export function HowItWorksPage() {
  return <main id="main">
    ...
    <section className="fresh-section"><div className="shell py-[clamp(5rem,10vw,10rem)]">{steps.map((step, i) => { const Icon = step.icon; return <Reveal key={step.n} className="grid gap-8 border-t border-ink py-12 lg:grid-cols-[0.2fr_0.65fr_1.15fr] lg:py-16"><div className="font-mono text-xs text-accent">{step.n} / 04</div><div><Icon className="h-10 w-10" /><h2 className="display mt-8 text-5xl sm:text-6xl">{step.title}</h2><p className="mt-3 font-semibold">{step.label}</p></div><div className="lg:pt-16"><p className="max-w-xl text-lg leading-relaxed text-ink-2">{step.body}</p><div className="mt-8 grid gap-2 sm:grid-cols-3">{step.meta.map((m) => <span key={m} className="border border-ink px-3 py-3 font-mono text-[10px] uppercase tracking-wider">{m}</span>)}</div></div></Reveal>; })}<div className="border-t border-ink" /></div></section>
