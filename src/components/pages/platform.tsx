...
function ProductFrame() {
  ...
  return <div className="border border-ink bg-panel"><div className="flex items-center justify-between border-b border-ink px-4 py-3"><span className="font-mono text-[11px] uppercase tracking-wider">Campus · active movement</span><span className="flex items-center gap-2 font-mono text-[10px]"><span className="live-dot" /> Live</span></div><div className="grid grid-cols-[1fr_1fr_70px_90px] border-b border-ink bg-bg-2 px-4 py-2 font-mono text-[9px] uppercase tracking-wider"><span>Student</span><span>Destination</span><span>Elapsed</span><span>Status</span></div>{rows.map((row, i) => <div key={row[0]} className="grid min-h-14 grid-cols-[1fr_1fr_70px_90px] items-center border-b border-line px-4 text-xs last:border-b-0"><span className="font-semibold">{row[0]}</span><span>{row[1]}</span><span className="font-mono">{row[2]}</span><span className={i === 1 ? "text-danger" : "text-go"}>{row[3]}</span></div>)}</div>;
}
