  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 border-b transition-colors duration-200 ${scrolled || open ? "border-line bg-panel" : "border-transparent bg-transparent"}`}>
      ...
      <nav className="shell flex h-20 items-center justify-between" aria-label="Primary navigation">
        <Link href="/" prefetch={false} className="flex min-h-12 items-center gap-3" aria-label="HallHop home">
        ...
        <div className="hidden items-center gap-6 lg:flex">
          {links.map((link) => {
            ...
            return <Link key={link.href} href={link.href} prefetch={false} aria-current={active ? "page" : undefined} className={`relative flex min-h-12 items-center text-sm font-semibold transition-colors duration-200 ${active ? "text-accent-2" : "text-ink hover:text-accent-2"}`}>{link.label}{active && <span className="absolute inset-x-0 bottom-1 h-0.5 bg-accent" />}</Link>;
          })}
        </div>

        <div className="flex items-center gap-3">
          <Link href="/contact" prefetch={false} className="btn btn-dark hidden sm:inline-flex">Contact sales</Link>
      ...
      {open && (
        <div id="mobile-menu" className="nav-menu-enter border-t border-line bg-panel lg:hidden">
          <div className="shell grid py-3">
            {links.map((link, index) => <Link key={link.href} href={link.href} prefetch={false} onClick={() => setOpen(false)} className="flex min-h-14 items-center justify-between border-b border-line text-lg font-semibold"><span>{link.label}</span><span className="font-mono text-xs">0{index + 1}</span></Link>)}
            <Link href="/contact" prefetch={false} onClick={() => setOpen(false)} className="btn btn-primary my-4 sm:hidden">Contact sales</Link>
          </div>
        </div>
      )}
    </header>
